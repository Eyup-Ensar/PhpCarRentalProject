<?php require "views/header.php"; ?>
    ...
    ?>
    <?php if (isset($veri["kazalar"])) : ?>
    <?php 

      if(isset($veri["sorgu"])):
        Session::set("excelkazasorgu", $veri["sorgu"]);
      endif;
      ...
      if(isset($veri["kazaarama"])) {
        $ayir = explode("-", $veri["kazaarama"]);
        if(count($ayir)>=4){
          $parca = explode("_", $veri["kazaarama"]);
          $tarih1 = $parca[0];
          $tarih2 = $parca[1];
        }else {
          $search = $veri["kazaarama"];
        }
      }

      $optValues = ["arac_id", "suruculer", "kaza_tarihi", "servis_adi", "aciklama"];

      $optNames = ["Araç", "Sürücüler", "Kaza Tarihi", "Servis Adı", "Açıklama"];

      $optClasses = ["", "", "optdate", "", ""];

    ?>
<link rel="stylesheet" href="<?php echo URLdesign; ?>/views/design/css/imagegallery.css">
  <div style="width:100%; padding: 10px; margin-top:10px">
      <div class="row mb-2 d-flex align-items-center">  
          ...
          <div class="col-4 col-md-6 d-xxl-none text-end">
            <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                <li><a class="dropdown-item" href="<?php echo URL; ?>/kazalar/kazaEkle">Kaza Ekle</a></li>
                <li>
                    <a type="submit" class="dropdown-item" href="<?php echo URL; ?>/kazalar/kazalarExcelAl">Excel Çıktısı Al</a>
                </li>
              </ul>
            </div>
          </div>
          <div class="col-12 col-xxl-4 col-md-6  mb-2 mb-xxl-0 mt-3 mt-xxl-0 pe-md-3">
            <form action="<?php echo URL."/kazalar/kazaArama" ?>" method="post">
              <div class="input-group">
                  ...
                  <input type="text" name="ara" class="form-control normalsearch" placeholder="Kaza Ara" value="<?php echo isset($_POST["ara"]) ? $_POST["ara"] : ($search!=="" ? $search : "") ?>">
                  <input type="date" name="tarih1" class="form-control datesearch d-none" placeholder="Kaza Ara" value="<?php echo isset($_POST["tarih1"]) ? $_POST["tarih1"] : ($tarih1!=="" ? $tarih1 : "") ?>">
                  <input type="date" name="tarih2" class="form-control datesearch d-none" placeholder="Kaza Ara" value="<?php echo isset($_POST["tarih2"]) ? $_POST["tarih2"] : ($tarih2!=="" ? $tarih2 : "") ?>">
          ...
          <div class="col-12 col-xxl-3 col-md-6 mb-2 mb-xxl-0 mt-3 mt-xxl-0 ps-md-3">
            <form action="<?php echo URL."/kazalar/kazaSirala" ?>" method="post">
              <div class="input-group">
      ...
      <table class="table">
        ...
        <?php 
          echo "<tbody>";
          foreach($veri["kazalar"] as $value):
                $kontrol = 0;
                echo '<tr class="tbodytd">';
                  if(!empty($value["resim"])) :
                  echo '<td class="gallery">';
                    $resimler = explode(",", $value["resim"]);
                    foreach ($resimler as $key => $val) :
                      echo $key===0 ? '<a href="'.URLdesign.'/views/design/images/'.$val.'">Galeriyi Görüntüle</a>' : '<a href="'.URLdesign.'/views/design/images/'.$val.'"></a>';
                    endforeach;
                  else: 
                    echo '<td class="text-danger">Resim Bulunamadı!</td>';
                  endif;
                  ...
                  echo '<td>'.date("d.m.Y", strtotime($value["kaza_tarihi"])).'</td>';
                  echo '<td>'.$value["servis_adi"].'</td>';
                  echo '<td>'.$value["aciklama"].'</td>';
          ?>
          <td class="d-flex justify-content-evenly">
            <a onclick="silmedenSor('<?php echo URL.'/kazalar/kazaSil/'.$value['id']; ?>'); return false" class="mr-2">
              <form method="post">
            ...
            <a href="<?php echo URL; ?>/kazalar/kazaGuncelle/<?php echo $value['id']?>">
            ...
            <a href="#" data-bs-toggle="modal" data-bs-target="#exampleModal" class="openModal" id="<?php echo $value['id']?>">
              <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" id="iconPrimary">
      ...
      <?php 
        if(isset($veri["kazaarama"]) && isset($veri["aramatercih"])) : 
          $link = '/kazalar/kazaArama/'.$Harici->seo($veri["aramatercih"]).'/'.$Harici->seo($veri["kazaarama"]).'/';
        elseif(isset($veri["siralamakriteri"]) && isset($veri["siralamatercih"])):
          $link = '/kazalar/kazaSirala/'.$Harici->seo($veri["siralamakriteri"]).'/'.$Harici->seo($veri["siralamatercih"]).'/';
        else:
          $link = '/kazalar/kazaListele/';
        endif;
...
<script src="<?php echo URLdesign; ?>/views/design/js/imagegallery.js"></script>
