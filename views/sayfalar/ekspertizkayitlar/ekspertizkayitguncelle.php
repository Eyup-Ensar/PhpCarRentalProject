...
<div  style="width:100%;">
  <nav class="container mt-3 mb-3">
    ...
    <div class="col-xl-12 col-md-12 mb-12 border-left-mvc text-left p-2 mb-2 navigasyonunanasi">
        ...
        / Ekspertiz Kayıt Güncelleme
    </div>
    <form method="post" action="<?php echo URL ?>/ekspertizkayitlar/ekspertizkayitGuncelleSon">
    <div class="mb-3 mt-3">
        <label>Araç:</label>
        <select name="arac_id" class="form-control">
          ...
          <?php 
            foreach ($veri["araclar"] as $value) {
              echo $value[0]===$veri["ekspertizkayit"][0]["arac_id"] ? 
      ...
      <div class="mb-3">
        ...
        <input type="text" name="ekspertiz_bayi" class="form-control" value="<?php echo $veri["ekspertizkayit"][0]["ekspertiz_bayi"];?>">
      </div>
      <div class="mb-3">
        <label>Tebliğ Tarihi:</label>
        <input type="date" name="ekspertiz_tarihi" class="form-control" value="<?php echo $veri["ekspertizkayit"][0]["ekspertiz_tarihi"];?>">
      </div>
      <div class="mb-3">
        <label>Açıklama:</label>
        <input type="date" name="aciklama" class="form-control" value="<?php echo $veri["ekspertizkayit"][0]["aciklama"];?>">
      </div>
      <div class="d-grid">
        <input type="hidden" name="id" value="<?php echo $veri["ekspertizkayit"][0]["id"];;?>">
        <button type="submit" name="submit" value="<?php $veri["ekspertizkayit"][0]["id"]?>" class="btn btn-primary">Güncelle</button>
      </div>
    </form>
  </nav>
</div>
