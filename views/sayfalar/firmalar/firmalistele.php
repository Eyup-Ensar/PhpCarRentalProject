<?php require "views/header.php"; ?>
  ...
  <?php if (isset($veri["firmalar"])) : ?>
    <?php 

      if(isset($veri["sorgu"])):
        Session::set("excelfirmasorgu", $veri["sorgu"]);
      endif;

      $search = "";

      if(isset($veri["firmaarama"])) {
        $search = $veri["firmaarama"];
      }

      $optValues = ["firma_ismi","vergi_dairesi","vergi_numarasi","yasak","borc","firma_durum"];

      $optNames = ["Firma İsmi","Vergi Dairesi","Vergi Numarası","Yasak","Borç","Firma Durum"];
    ...
    <div style="width:100%; padding: 10px; margin-top:10px">
      <div class="row mb-2 d-flex align-items-center">  
        ...
        <div class="col-4 col-md-6 d-xxl-none text-end">
          <div class="dropdown">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
              <li><a class="dropdown-item" href="<?php echo URL; ?>/firmalar/firmaEkle">Firma Ekle</a></li>
              <li><a class="dropdown-item" href="<?php echo URL; ?>/firmalar/toplufirmaEkle">Çoklu Firma Ekle</a></li>
              <li>
                  <a type="submit" class="dropdown-item" href="<?php echo URL; ?>/firmalar/firmalarExcelAl">Excel Çıktısı Al</a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-12 col-xxl-4 col-md-6  mb-2 mb-xxl-0 mt-3 mt-xxl-0 pe-md-3">
          <form action="<?php echo URL."/firmalar/firmaArama" ?>" method="post">
            <div class="input-group">
                ...
                <input type="text" name="ara" class="form-control normalsearch" placeholder="Firma Ara" value="<?php echo isset($_POST["ara"]) ? $_POST["ara"] : ($search!=="" ? $search : "") ?>">
        ...
        <div class="col-12 col-xxl-3 col-md-6 mb-2 mb-xxl-0 mt-3 mt-xxl-0 ps-md-3">
          <form action="<?php echo URL."/firmalar/firmaSirala" ?>" method="post">
            <div class="input-group">
      ...
      <table class="table"> 
        <thead>
          ...
          <?php 
            ...
            <?php foreach($veri["firmalar"] as $value):
              echo '<tbody>';
                echo '<tr class="tbodytd" id="'.$value['cus_id'].'">';
                  echo '<td>'.$value["firma_ismi"].'</td>';
                  echo '<td>'.$value["vergi_dairesi"].'</td>';
                  echo '<td>'.$value["vergi_numarasi"].'</td>';
                  echo '<td>'.$value["yasak"].'</td>';
                  echo '<td>'.$value["borc"].'</td>';
                  echo '<td class="'.($value["firma_durum"]=="aktif" ? "text-success" : "text-danger").'">'.$value["firma_durum"].'</td>';
                ?>
              <td class="d-flex justify-content-evenly">
                <a onclick="silmedenSor('<?php echo URL.'/firmalar/firmaSil/'.$value['cus_id']; ?>'); return false" class="mr-2">
                  <form method="post">
                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#ff5353" id="iconDanger">
                        <path d="M0 0h24v24H0V0z" fill="none"/>
                        <path d="M16 9v10H8V9h8m-1.5-6h-5l-1 1H5v2h14V4h-3.5l-1-1zM18 7H6v12c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7z"/>
                    </svg>
                  </form>
                </a>
                <a href="<?php echo URL; ?>/firmalar/firmaGuncelle/<?php echo $value['cus_id']?>">
                ...
                <a href="#" data-bs-toggle="modal" data-bs-target="#exampleModal" class="openModal" id="<?php echo $value['cus_id']?>">
                  <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" fill="#0d6efd" id="iconPrimary">
      ...
      <?php 
        if(isset($veri["firmaarama"]) && isset($veri["aramatercih"])) : 
          $link = '/firmalar/firmaArama/'.$Harici->seo($veri["aramatercih"]).'/'.$Harici->seo($veri["firmaarama"]).'/';
        elseif(isset($veri["siralamakriteri"]) && isset($veri["siralamatercih"])):
          $link = '/firmalar/firmaSirala/'.$Harici->seo($veri["siralamakriteri"]).'/'.$Harici->seo($veri["siralamatercih"]).'/';
        else:
          $link = '/firmalar/firmaListele/';
        endif;
