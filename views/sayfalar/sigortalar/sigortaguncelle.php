...
<div  style="width:100%;">
  <nav class="container mt-3 mb-3">
    ...
    <div class="col-xl-12 col-md-12 mb-12 border-left-mvc text-left p-2 mb-2 navigasyonunanasi">
        <a href="<?php echo URL ?>/sigortalar/sigortaListele" class="navigasyon">Sigortalar</a> 
        / Sigorta Güncelleme
    </div>
    <form method="post" action="<?php echo URL ?>/sigortalar/sigortaGuncelleSon">
    <div class="mb-3 mt-3">
        <label>Araç:</label>
        <select name="arac_id" class="form-control">
          ...
          <?php 
            foreach ($veri["araclar"] as $value) {
              echo $value[0]===$veri["sigorta"][0]["arac_id"] ? 
      ...
      <div class="mb-3">
        ...
        <input type="text" name="sigorta_turu" class="form-control" value="<?php echo $veri["sigorta"][0]["sigorta_turu"];?>">
      </div>
      <div class="mb-3">
        <label>Başlangıç Tarihi:</label>
        <input type="date" name="baslangic_tarihi" class="form-control" value="<?php echo $veri["sigorta"][0]["baslangic_tarihi"];?>">
      </div>
      <div class="mb-3">
        <label>Bitiş Tarihi:</label>
        <input type="date" name="bitis_tarihi" class="form-control" value="<?php echo $veri["sigorta"][0]["bitis_tarihi"];?>">
      </div>
      <div class="d-grid">
        <input type="hidden" name="id" value="<?php echo $veri["sigorta"][0]["id"];?>">
        <button type="submit" name="submit" value="<?php $veri["sigorta"][0]["id"]?>" class="btn btn-primary">Güncelle</button>
      </div>
    </form>
  </nav>
</div>
