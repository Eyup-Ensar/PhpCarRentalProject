<?php require "views/header.php"; ?>
    ...
    ?>
    <?php if (isset($veri["araclar"])) : ?>
    <?php 

      if (isset($veri["sorgu"])) : 

        Session::set("excelaracsorgu", $veri["sorgu"]);

      endif;


      if (isset($veri["sigortalar"])) : 

        Session::set("excelaracsigortasorgu", $veri["sigortalar"]);
      ...
      if(isset($veri["aracarama"])) {
        $ayir = explode("-", $veri["aracarama"]);
        if(count($ayir)>=4){
          $parca = explode("_", $veri["aracarama"]);
          $tarih1 = $parca[0];
          $tarih2 = $parca[1];
        }else {
          $search = $veri["aracarama"];
        }
      }

      $optValues = ["plaka","marka", "model","yakit_tipi","sanziman","durum","kilometre","bakim","gelecek_bakim_km","yil","renk","muayene_tarihi",
      "sigorta_bas_tarihi","sigorta_bit_tarihi","kasko_bas_tarihi","kasko_bit_tarihi","km_tarihi","sase_no","kullanici","telefon"];

      $optNames = ["Plaka","Marka", "Model","Yakıt Tipi","Şanzıman","Durum","Kilometre","Bakıma Girdiği KM","gelecek_bakim_km","Araç Yılı","Araç Rengi","Muayene Tarihi",
      "Sigorta Başlangıç Tarihi","Sigorta Bitiş Tarihi" ,"Kasko Başlangıç Tarihi","Kasko Bitiş Tarihi" ,"KM Tarihi","Şase No","Kullanici","Telefon"];

      $optClasses = ["", "","","","","","","","","","","optdate","optdate","optdate","optdate","optdate","optdate","optdate","","",""];
  ...
  <div style="width:100%; padding: 10px; margin-top:10px">
      <div class="row mb-2 d-flex align-items-center">  
          ...
          <div class="col-4 col-md-6 d-xxl-none text-end">
            <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                <li><a class="dropdown-item" href="<?php echo URL; ?>/araclar/aracEkle">Araç Ekle</a></li>
                <li><a class="dropdown-item" href="<?php echo URL; ?>/kiralar/kiraBaslat">Kira Başlat</a></li>
                <li><a class="dropdown-item" href="<?php echo URL; ?>/araclar/topluAracEkle">Çoklu Araç Ekle</a></li>
                <li>
                    <a type="submit" class="dropdown-item" href="<?php echo URL; ?>/araclar/araclarExcelAl">Excel Çıktısı Al</a>
                </li>
              </ul>
            </div>
          </div>
          <div class="col-12 col-xxl-4 col-md-6  mb-2 mb-xxl-0 mt-3 mt-xxl-0 pe-md-3">
            <form action="<?php echo URL."/araclar/aracArama" ?>" method="post">
              <div class="input-group">
                  ...
                  <input type="text" name="ara" class="form-control normalsearch" placeholder="Araç Ara" value="<?php echo isset($_POST["ara"]) ? $_POST["ara"] : ($search!=="" ? $search : "") ?>">
                  <input type="date" name="tarih1" class="form-control datesearch d-none" placeholder="Araç Ara" value="<?php echo isset($_POST["tarih1"]) ? $_POST["tarih1"] : ($tarih1!=="" ? $tarih1 : "") ?>">
                  <input type="date" name="tarih2" class="form-control datesearch d-none" placeholder="Araç Ara" value="<?php echo isset($_POST["tarih2"]) ? $_POST["tarih2"] : ($tarih2!=="" ? $tarih2 : "") ?>">
          ...
          <div class="col-12 col-xxl-3 col-md-6 mb-2 mb-xxl-0 mt-3 mt-xxl-0 ps-md-3">
            <form action="<?php echo URL."/araclar/aracSirala" ?>" method="post">
              <div class="input-group">
      ...
      <table class="table">
        ...
        <?php 
          ...
          foreach($veri["araclar"] as $value):
              $bakim_gecmis = $value["bakim"] + 10000;
              $sayac = 0;
              $tsigortacontrol = 0;
              $ksigortacontrol = 0;
                echo '<tr class="tbodytd" id="'.$value['id'].'">';
                  echo '<td>'.$value["plaka"].'</td>';
                  echo '<td>'.$value["marka"].'</td>';
                  echo '<td>'.$value["model"].'</td>';
                  echo '<td>'.$value["yakit_tipi"].'</td>';
                  echo '<td>'.$value["sanziman"].'</td>';
                  echo '<td>'.$value["durum"].'</td>';
                  echo '<td>'.$value["kilometre"].'</td>';
                  echo '<td>'.$value["bakim"].'</td>';
                  echo ($bakim_gecmis < $value["kilometre"]) ? 
                  '<td>'.$bakim_gecmis.'</td>' : 
                  '<td>'.$bakim_gecmis.'</td>';
                  echo '<td>'.$value["yil"].'</td>';
                  echo '<td>'.$value["renk"].'</td>';
              if (empty($value['muayene_tarihi'])) {
                $sayac += 1;
                echo '<td class="text-danger"">Girilmemiş</td>';
              } else {
                $sozbit = new DateTime($value['muayene_tarihi']);
              ...
              foreach ($veri["sigortalar"] as $key => $val) :
                if($value["plaka"]==$val[0]) :
                    if($val[1]=="Trafik Sigortası") :
                      echo '<td>'.date("d.m.Y", strtotime($val[2])).'</td>';
                      echo '<td>'.date("d.m.Y", strtotime($val[3])).'</td>';
                      $tsigortacontrol++;
                    endif;
                endif;
              endforeach;
            ...
            echo '<td>'.date("d.m.Y", strtotime($value["km_tarihi"])).'</td>';
            echo '<td>'.$value["sase_no"].'</td>';
            echo '<td>'.$value["kullanici"].'</td>';
            echo '<td>'.$value["telefon"].'</td>';
          ?>
          <td class="d-flex justify-content-evenly">
            <a onclick="silmedenSor('<?php echo URL.'/araclar/aracSil/'.$value['id']; ?>'); return false" class="mr-2">
              <form method="post">
            ...
            <a href="<?php echo URL; ?>/araclar/aracGuncelle/<?php echo $value['id']?>">
      ...
      <?php 
        if(isset($veri["aracarama"]) && isset($veri["aramatercih"])) : 
          $link = '/araclar/aracArama/'.$Harici->seo($veri["aramatercih"]).'/'.$Harici->seo($veri["aracarama"]).'/';
        elseif(isset($veri["siralamakriteri"]) && isset($veri["siralamatercih"])):
          $link = '/araclar/aracSirala/'.$Harici->seo($veri["siralamakriteri"]).'/'.$Harici->seo($veri["siralamatercih"]).'/';
        else:
          $link = '/araclar/aracListele/';
        endif;
