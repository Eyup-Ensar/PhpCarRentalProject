<?php require "views/header.php"; ?>
  ...
  <?php if (isset($veri["raporlar"])) : ?>
    <?php 

      if(isset($veri["sorgu"])):
        Session::set("excelraporsorgu", $veri["sorgu"]);
      endif;

      $tarih1 = "";
      $tarih2 = "";
      $search = "";
      if(isset($veri["raporarama"])) {
        $ayir = explode("-", $veri["raporarama"]);
        if(count($ayir)>=4){
          $parca = explode("_", $veri["raporarama"]);
          $tarih1 = $parca[0];
          $tarih2 = $parca[1];
        }else {
          $search = $veri["raporarama"];
        }
      }

      $optValues = ["arac_id", "firma", "testar", "teskm", "ucret", "tesnot"];

      $optNames = ["Araç", "Firma","Teslim Tarihi","Teslim KM", "Ödenen Ücret", "Teslim Not"];
    ...
    <div style="width:100%; padding: 10px; margin-top:10px">
      <div class="row mb-2 d-flex align-items-center">  
        ...
        <div class="col-4 col-md-6 d-xxl-none text-end">
          <div class="dropdown">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
              <li>
                  <a type="submit" class="dropdown-item" href="<?php echo URL; ?>/raporlar/raporlarExcelAl">Excel Çıktısı Al</a>
              </li>
            </ul>
          </div>
        </div>
        <div class="col-12 col-xxl-4 col-md-6  mb-2 mb-xxl-0 mt-3 mt-xxl-0 pe-md-3">
          <form action="<?php echo URL."/raporlar/raporArama" ?>" method="post">
            <div class="input-group">
              ...
              <input type="text" name="ara" class="form-control normalsearch" placeholder="Rapor Ara" value="<?php echo isset($_POST["ara"]) ? $_POST["ara"] : ($search!=="" ? $search : "") ?>">
        ...
        <div class="col-12 col-xxl-3 col-md-6 mb-2 mb-xxl-0 mt-3 mt-xxl-0 ps-md-3">
          <form action="<?php echo URL."/raporlar/raporSirala" ?>" method="post">
            <div class="input-group">
      ...
      <table class="table">
        <thead>
          <tr class="theadtd">
            ...
            <?php foreach($veri["raporlar"] as $value):
              $kontrol = 0;
              echo '<tbody>';
                echo '<tr class="tbodytd" id="'.$value['id'].'">';
                  ...
                  echo '<td>'.date("d.m.Y", strtotime($value["testar"])).'</td>';
                  echo '<td>'.$value["teskm"].'</td>';
                  echo '<td>'.number_format($value["ucret"],2,",",".").' TL</td>';
                  echo '<td>'.$value["tesnot"].'</td>';
                ?>
              <td class="d-flex justify-content-evenly">
                <a onclick="silmedenSor('<?php echo URL.'/raporlar/raporSil/'.$value['id']; ?>'); return false" class="mr-2">
                  <form method="post">
                ...
                <a href="<?php echo URL; ?>/raporlar/raporGuncelle/<?php echo $value['id']?>">
      ...
      <?php 
        if(isset($veri["raporarama"]) && isset($veri["aramatercih"])) : 
          $link = '/raporlar/raporArama/'.$Harici->seo($veri["aramatercih"]).'/'.$Harici->seo($veri["raporarama"]).'/';
        elseif(isset($veri["siralamakriteri"]) && isset($veri["siralamatercih"])):
          $link = '/raporlar/raporSirala/'.$Harici->seo($veri["siralamakriteri"]).'/'.$Harici->seo($veri["siralamatercih"]).'/';
        else:
          $link = '/raporlar/raporListele/';
        endif;
