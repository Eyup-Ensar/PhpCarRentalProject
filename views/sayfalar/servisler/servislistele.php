<?php require "views/header.php"; ?>
    ...
    ?>
    <?php if (isset($veri["servisler"])) : ?>
    <?php 

      if(isset($veri["sorgu"])):
        Session::set("excelservissorgu", $veri["sorgu"]);
      endif;
      ...
      if(isset($veri["servisarama"])) {
        $ayir = explode("-", $veri["servisarama"]);
        if(count($ayir)>=4){
          $parca = explode("_", $veri["arama"]);
          $tarih1 = $parca[0];
          $tarih2 = $parca[1];
        }else {
          $search = $veri["servisarama"];
        }
      }

      $optValues = ["arac_id", "servis_bayi_adi", "yapilan_islem", "islem_nedeni", "servis_giris_tarihi", "servis_cikis_tarihi", "servis_giris_km", "servis_tutari"];

      $optNames = ["Araç", "Servis Bayi Adı", "Yapılan İşlem", "İşlem Nedeni", "Servis Giriş Tarihi", "Servis Çıkış Tarihi", "Servis Giriş KM", "Servis Tutarı"];
  ...
  <div style="width:100%; padding: 10px; margin-top:10px">
      <div class="row mb-2 d-flex align-items-center">  
          ...
          <div class="col-4 col-md-6 d-xxl-none text-end">
            <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                <li><a class="dropdown-item" href="<?php echo URL; ?>/servisler/servisEkle">Servis Ekle</a></li>
                <li>
                    <a type="submit" class="dropdown-item" href="<?php echo URL; ?>/servisler/servislerExcelAl">Excel Çıktısı Al</a>
                </li>
              </ul>
            </div>
          </div>
          <div class="col-12 col-xxl-4 col-md-6  mb-2 mb-xxl-0 mt-3 mt-xxl-0 pe-md-3">
            <form action="<?php echo URL."/servisler/servisArama" ?>" method="post">
          ...
          <div class="col-12 col-xxl-3 col-md-6 mb-2 mb-xxl-0 mt-3 mt-xxl-0 ps-md-3">
            <form action="<?php echo URL."/servisler/servisSirala" ?>" method="post">
              <div class="input-group">
      ...
      <table class="table">
        ...
        <?php 
          ...
          $kontrol = 0;
          echo "<tbody>";
          foreach($veri["servisler"] as $value):
                echo '<tr class="tbodytd" id="'.$value['id'].'">';
                  ...
                  echo '<td>'.$value["servis_bayi_adi"].'</td>';
                  echo '<td>'.$value["yapilan_islem"].'</td>';
                  echo '<td>'.$value["islem_nedeni"].'</td>';
                  echo '<td>'.date('d.m.Y', strtotime($value['servis_giris_tarihi'])).'</td>';
                  echo '<td>'.date('d.m.Y', strtotime($value['servis_cikis_tarihi'])).'</td>';
                  echo '<td>'.$value["servis_giris_km"].'</td>';
                  echo '<td>'.number_format($value["servis_tutari"],2,",",".").' TL</td>';
          ?>
          <td class="d-flex justify-content-evenly">
            <a onclick="silmedenSor('<?php echo URL.'/servisler/servisSil/'.$value['id']; ?>'); return false" class="mr-2">
              <form method="post">
            ...
            <a href="<?php echo URL; ?>/servisler/servisGuncelle/<?php echo $value['id']?>">
      ...
      <?php 
        if(isset($veri["servisarama"]) && isset($veri["aramatercih"])) : 
          $link = '/servisler/servisArama/'.$Harici->seo($veri["aramatercih"]).'/'.$Harici->seo($veri["servisarama"]).'/';
        elseif(isset($veri["siralamakriteri"]) && isset($veri["siralamatercih"])):
          $link = '/servisler/servisSirala/'.$Harici->seo($veri["siralamakriteri"]).'/'.$Harici->seo($veri["siralamatercih"]).'/';
        else:
          $link = '/servisler/servisListele/';
        endif;
