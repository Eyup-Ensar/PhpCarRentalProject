<?php require "views/header.php"; ?>
    ...
    ?>
    <?php if (isset($veri["yakitlar"])) : ?>
    <?php 

      if(isset($veri["sorgu"])):
        Session::set("excelyakitsorgu", $veri["sorgu"]);
      endif;
      ...
      if(isset($veri["yakitarama"])) {
        $ayir = explode("-", $veri["yakitarama"]);
        if(count($ayir)>=4){
          $parca = explode("_", $veri["arama"]);
          $tarih1 = $parca[0];
          $tarih2 = $parca[1];
        }else {
          $search = $veri["yakitarama"];
        }
      }

      $optValues = ["arac_id", "surucu_id", "istasyon", "fatura_tarihi", "yakit_alim_km", "birim_fiyat", "miktar", "iskonto", "toplam_tutar"];

      $optNames = ["Araç", "Sürücü", "İstasyon", "Fatura Tarihi", "Yakıt Alım KM", "Birim Fiyat", "Miktar", "İskonto", "Toplam Tutar"];
  ...
  <div style="width:100%; padding: 10px; margin-top:10px">
      <div class="row mb-2 d-flex align-items-center">  
          ...
          <div class="col-4 col-md-6 d-xxl-none text-end">
            <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                <li><a class="dropdown-item" href="<?php echo URL; ?>/yakitlar/yakitEkle">yakit Ekle</a></li>
                <li>
                    <a type="submit" class="dropdown-item" href="<?php echo URL; ?>/yakitlar/yakitlarExcelAl">Excel Çıktısı Al</a>
                </li>
              </ul>
            </div>
          </div>
          <div class="col-12 col-xxl-4 col-md-6  mb-2 mb-xxl-0 mt-3 mt-xxl-0 pe-md-3">
            <form action="<?php echo URL."/yakitlar/yakitArama" ?>" method="post">
              <div class="input-group">
                  ...
                  <input type="text" name="ara" class="form-control normalsearch" placeholder="yakit Ara" value="<?php echo isset($_POST["ara"]) ? $_POST["ara"] : ($search!=="" ? $search : "") ?>">
                  <input type="date" name="tarih1" class="form-control datesearch d-none" placeholder="yakit Ara" value="<?php echo isset($_POST["tarih1"]) ? $_POST["tarih1"] : ($tarih1!=="" ? $tarih1 : "") ?>">
                  <input type="date" name="tarih2" class="form-control datesearch d-none" placeholder="yakit Ara" value="<?php echo isset($_POST["tarih2"]) ? $_POST["tarih2"] : ($tarih2!=="" ? $tarih2 : "") ?>">
          ...
          <div class="col-12 col-xxl-3 col-md-6 mb-2 mb-xxl-0 mt-3 mt-xxl-0 ps-md-3">
            <form action="<?php echo URL."/yakitlar/yakitSirala" ?>" method="post">
              <div class="input-group">
          ...
          <div class="d-none d-xxl-block col-xxl-2 text-end">
            <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                <li><a class="dropdown-item" href="<?php echo URL; ?>/yakitlar/yakitEkle">Yakıt Ekle</a></li>
                <li>
                    <a type="submit" class="dropdown-item" href="<?php echo URL; ?>/yakitlar/yakitlarExcelAl">Excel Çıktısı Al</a>
                </li>
              </ul>
      ...
      <table class="table">
        ...
        <?php 
          ...
          $kontrol = 0;
          echo "<tbody>";
          foreach($veri["yakitlar"] as $value):
                $kontrol = 0;
                echo '<tr class="tbodytd" id="'.$value['id'].'">';
                  ...
                  echo '<td>'.$value["istasyon"].'</td>';
                  echo '<td>'.date("d.m.Y", strtotime($value["fatura_tarihi"])).'</td>';
                  echo '<td>'.$value["yakit_alim_km"].'</td>';
                  echo '<td>'.number_format($value["birim_fiyat"],2,",",".").' TL</td>';
                  echo '<td>'.number_format($value["miktar"],2,",",".").' TL</td>';
                  echo '<td>'.number_format($value["iskonto"],2,",",".").' TL</td>';
                  echo '<td>'.number_format($value["toplam_tutar"],2,",",".").' TL</td>';
          ?>
          <td class="d-flex justify-content-evenly">
            <a onclick="silmedenSor('<?php echo URL.'/yakitlar/yakitSil/'.$value['id']; ?>'); return false" class="mr-2">
              <form method="post">
            ...
            <a href="<?php echo URL; ?>/yakitlar/yakitGuncelle/<?php echo $value['id']?>">
      ...
      <?php 
        if(isset($veri["yakitarama"]) && isset($veri["aramatercih"])) : 
          $link = '/yakitlar/yakitArama/'.$Harici->seo($veri["aramatercih"]).'/'.$Harici->seo($veri["yakitarama"]).'/';
        elseif(isset($veri["siralamakriteri"]) && isset($veri["siralamatercih"])):
          $link = '/yakitlar/yakitSirala/'.$Harici->seo($veri["siralamakriteri"]).'/'.$Harici->seo($veri["siralamatercih"]).'/';
        else:
          $link = '/yakitlar/yakitListele/';
        endif;
